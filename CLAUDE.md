...
- Sala de espera (módulo `waiting_room`, apagado por defecto, se activa en Administración → Módulos): al registrar la llegada (`POST /api/shifts/{id}/arrival`, `POST /api/walk-ins`) se emite un número de sala diario (`WaitingTicket`, `@@unique([date, number])`, correlativo con `SELECT MAX … FOR UPDATE` + reintento en P2002) que identifica al paciente sin exponer su nombre y no promete orden. Un turno/walk-in tiene un solo ticket: repetir la llegada devuelve el mismo número, deshacerla lo anula (`VOID`) y volver a registrarla lo reabre. La pantalla pública solo lee `WaitingTicket` (números y consultorios). Con el módulo apagado todo se comporta como antes (`ticket: null`)
