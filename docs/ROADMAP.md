...
| D3 | Cobros y liquidación por obra social | ☐ | No hay modelo de pagos (copagos, caja diaria, liquidación). Desde el 25-sep-2026 cada turno guarda con qué cobertura se atiende (`Shift.coverageInsuranceId` / `isPrivate`, `lib/shift-coverage.ts`), base para liquidar por obra social; los turnos anteriores no la tienen. |
| D4 | Sala de espera y llamado (módulo `waiting_room`) | ☑ | Hecho el 25-sep-2026 (diseño en [`SALA-DE-ESPERA.md`](SALA-DE-ESPERA.md)): módulo apagado por defecto, `WaitingTicket` con número de sala diario al registrar llegadas y walk-ins, llamado a consultorio con consultorio desde recepción y desde «Turnos de hoy» del médico, «volver a llamar», pestaña «En consulta», cierre del número al finalizar/ausentar/cancelar, pantalla pública `/sala` con clave de dispositivo (solo números y consultorios, campanilla y voz opcionales) y su administración en Configuración → Consultorio. El 25-sep-2026 se sumó la ficha en modo consulta: «Atender», «Llamar» e «Iniciar consulta» abren la ficha del paciente con la barra «Consulta en curso» (número, consultorio, minutos, evolución vinculada al turno, receta, orden, volver a llamar, finalizar) y las filas en consulta tienen «Continuar». Quedan para más adelante: prefijo por profesional y modo «Apellido, N.», impresora de tickets, push al celular (D5). |
