...
| D4 | Sala de espera y llamado (módulo `waiting_room`) | ◐ | Diseño en [`SALA-DE-ESPERA.md`](SALA-DE-ESPERA.md). Fases 1 y 2 hechas el 25-sep-2026: módulo apagado por defecto, `WaitingTicket`, número de sala al registrar llegadas y walk-ins, badges y diálogo con el número en grande; llamado a consultorio con diálogo de consultorio desde recepción y desde «Turnos de hoy» del médico, `recall`, pestaña «En consulta» con «Volver a llamar» / «No se presentó», cierre del número al finalizar, ausentar o cancelar. Pendiente: fase 3 (clave de dispositivo, feed público, pantalla `/sala`, sonido). |
