...
- **Documentación de la API**: generada del código (`lib/openapi`). `pnpm api:docs` regenera `contracts/openapi.json` y `contracts/openapi.mobile.json`; visor en `/dashboard/administracion/api-docs` (admin); guía para la app móvil y generación del cliente Dart en `docs/API-MOBILE.md`.
